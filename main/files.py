...
import win32com.client
def download_reports(p_name,d_name,d_spes,date,time,pres,report):
    url='file:///C:/programming/Hackathontyl/Hackathontyl/'
    ...
    def precipitation(name,text):
        document = docx.Document()
        document.add_heading(r'Doctor prescription  ', 0)
        document.add_heading(text,1)
        

        name=name.replace(' ','_')+'_prescription'
        document.save(name+'.docx')
        webbrowser.get(g_path).open(url+name+'.docx')
    ...
    report(p_name,d_name,d_spes,"""The brain is the control center of the body. It controls thoughts, memory, speech, and movement. It regulates the function of many organs. When the brain is healthy, it works quickly and automatically. However, when problems occur, the results can be devastating.
...
def billing(name="babu",email="babu@gmail.com"):
    url='file:///C:/programming/Hackathontyl/Hackathontyl/'
    g_path='C:/Program Files/Google/Chrome/Application/chrome.exe %s'
    document = docx.Document()
    val = [("Paracetamol(400mg)",2,50),("Dolo-650(500mg)",2,100),("Betadine(15mg)",1,80),("Allegra(120mg)",1,180)]
    document.add_heading('Invoice', 0)
    p1 = document.add_heading('Dear ',1)
    p1.add_run(name).bold=True
    p1.add_run(',')
    ...
    document.add_paragraph('')
    x=0;
    for product, unit, price in val:
        row_cells = table.add_row().cells
        row_cells[0].text = product
        row_cells[1].text = f'{unit:,.2f}'
        row_cells[2].text = f'{price:,.2f}'
        x+=unit * price
        row_cells[3].text = f'{unit * price:,.2f} Rs'

    row_cells = table.add_row().cells
    
    row_cells = table.add_row().cells
    row_cells[2].text = "Tax(20%)"
    row_cells[2].paragraphs[0].runs[0].font.bold = True
    row_cells[3].text = f'{((20)/100) * x:,.2f} Rs'
    tax=((20)/100)* x

    row_cells = table.add_row().cells
    row_cells[2].text = "Total Price + Tax"
    row_cells[2].paragraphs[0].runs[0].font.bold = True
    row_cells[3].text = f'{x+tax:,.2f} Rs'
...
Customers desirous of availing input GST credit are requested to create a Business account and purchase on iMediCare.in/business from Business eligible offers Please note that this invoice is not a demand for payment*''')
    ...
    document.save(name+'_invoice'+'.docx')
    webbrowser.get(g_path).open(url+name+"_invoice"+'.docx')
